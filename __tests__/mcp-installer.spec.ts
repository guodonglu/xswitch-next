...
function runInstaller(
  ...
  return spawnSync(process.execPath, [INSTALLER, ...args, '--no-register'], {
    encoding: "utf8",
    env: {
      ...process.env,
...
describe("XSwitch MCP installer", () => {
  beforeAll(() => {
    const result = spawnSync(
      process.execPath,
      [process.env.npm_execpath!, "--prefix", "mcp", "run", "build"],
      { encoding: "utf8" }
    );
  ...
  it("installs and uninstalls inside a custom user data directory", () => {
    ...
    expect(manifest.path).toBe(path.join(installDirectory, process.platform === 'win32' ? 'native-host.cmd' : 'native-host'));
