...
async function waitForSocket(socketPath: string) {
  // MCP initialization completes only after listen(); named pipes have no filesystem entry.
  ...
  it(
    ...
    async () => {
      ...
      const socketPath = process.platform === "win32"
        ? `\\\\.\\pipe\\${path.basename(tempDirectory)}`
        : path.join(tempDirectory, "bridge.sock");
