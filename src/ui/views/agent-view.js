...
const actionNames = {
  ...
  GROUP_COPY: '复制规则组',
  GROUP_MOVE: '调整分组顺序',
  CONFIG_ENABLED: '切换总开关',
  OPTIONS_SET: '修改选项',
  BACKUP_RESTORE: '恢复备份',
  CONFIG_IMPORT: '导入配置',
  LEGACY_UPSERT: '修改原版规则组',
  CONFIG_CLEAR: '清除规则',
};
...
export function agentView(ctx) {
  ...
  const copy = () => ctx.guard(async () => {
    await navigator.clipboard.writeText(setupPrompt(chrome.runtime.id));
    ctx.toast({ message: '配置指令已复制' });
  });

  const view = el('div', { class: 'space-y-4' },
    el('section', { class: 'panel space-y-3 p-4' },
      el('h2', { class: 'text-sm font-semibold' }, '连接 AI Agent'),
      ...
      el('div', { class: 'flex flex-wrap gap-2' },
        button('复制 Agent 配置 Prompt', copy, 'btn-primary'),
        button('重新连接', () => ctx.guard(async () => {
          await ctx.request('AGENT_RECONNECT');
          ctx.toast({ message: '正在重新连接本机服务' });
        }))
      ),
  ...
  const rollback = (operation) => ctx.guard(async () => {
    const timeStr = new Date(operation.timestamp).toLocaleTimeString();
    const actionDesc = actionNames[operation.action] ?? '该操作';
    const detail = operation.change?.rule ? `「${operation.change.rule}」` : operation.change?.group ? `「${operation.change.group}」` : '';
    const confirmed = await dialog({
      title: '回滚配置',
      message: `确定要回滚到 ${timeStr} 执行${actionDesc}${detail}之前的状态吗？当前配置将被备份。`,
      confirmLabel: '回滚',
      danger: true,
    });
    if (confirmed) {
      await ctx.change('BACKUP_RESTORE', { backupId: operation.backupId });
      ctx.toast({ message: `已成功回滚至 ${timeStr} 状态` });
    }
  });

  const recentHistory = (ctx.state.history ?? []).slice(0, 10);
  const history = el('section', { class: 'space-y-3' },
    el('div', { class: 'flex items-center justify-between gap-2' },
      el('h2', { class: 'text-sm font-semibold' }, '最近操作'),
      recentHistory.length > 0 && el('span', { class: 'muted text-xs' }, `显示最近 ${recentHistory.length} 次`)
    )
  );

  if (!recentHistory.length) {
    history.append(el('p', { class: 'muted py-8 text-center text-xs' }, '还没有操作记录。'));
  }

  for (const operation of recentHistory) {
    const hasBackup = ctx.state.backups?.some((b) => b.id === operation.backupId);
    history.append(el('article', { class: 'panel space-y-2 p-3' },
      el('div', { class: 'flex items-center justify-between gap-2 text-[10px]' },
        el('div', { class: 'flex items-center gap-2' },
          el('span', { class: 'font-medium text-primary' }, operation.source === 'agent' ? 'AI Agent' : operation.source === 'import' ? '配置导入' : '手动操作'),
          el('time', { class: 'muted' }, new Date(operation.timestamp).toLocaleString())
        ),
        hasBackup && button('回滚', () => rollback(operation), 'text-xs py-0.5 px-2 text-text-secondary hover:text-primary')
      ),
