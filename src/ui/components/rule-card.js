...
export function createRuleCard(rule, actions) {
  ...
  if (rule.type === 'redirect') card.append(el('span', { class: 'my-1 block text-primary', 'aria-hidden': 'true' }, '↓'),
    el('p', { class: 'break-all font-mono text-xs leading-5 text-primary' }, rule.destination.value || '（空字符串替换）'));
  card.append(el('div', { class: 'mt-3 flex flex-wrap items-center justify-between gap-2' },
    el('div', { class: 'muted flex gap-2 text-[10px]' }, rule.match.mode === 'regex' && el('span', { class: 'rounded bg-warning/15 px-1.5 py-0.5 font-medium text-warning' }, 'Regex'),
