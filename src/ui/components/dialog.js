...
export function dialog({ title, message, inputValue, confirmLabel = '确定', danger = false }) {
  return new Promise((resolve) => {
    ...
    const modal = el('dialog', { class: 'panel m-auto w-[calc(100%-2rem)] max-w-sm p-5 text-zinc-900 shadow-xl backdrop:bg-black/40 dark:text-zinc-100', 'aria-labelledby': 'dialog-title' }, form);
    let answer = null;
    form.append(el('div', { class: 'flex justify-end gap-2' }, button('取消', () => modal.close()),
      ...
      }, danger ? 'border-red-500 text-red-600' : 'btn-primary')));
